...
const InformeStandard = ({ route, navigation }) => {
  ...
  const validarRespuestas = () => {
    ...
    if (respuestasIncompletas) {
      Alert.alert(
        "Error",
        'Debe responder todas las preguntas de tipo "Sí" o "No" antes de continuar.'
      );
      return false;
    }
    return true;
  ...
  const enviarDatosAGuardarInforme = async () => {
    ...
    try {
      const response = await fetch(
        "https://isorga.com/api/guardarInforme.php",
        {
          method: "POST",
          body: formData,
          headers: {
                  ...
                  <View style={styles.radioGroupContainer}>
                    <View style={styles.radioGroup}>
                      <TouchableOpacity
                        ...
                        style={[
                          styles.radioButton,
                          selectedAnswers[pregunta.id]?.respuesta === 1 &&
                      ...
                      <TouchableOpacity
                        ...
                        style={[
                          styles.radioButton,
                          selectedAnswers[pregunta.id]?.respuesta === 0 &&
                            styles.radioButtonSelected,
                        ]}
                      >
...
const styles = StyleSheet.create({
  ...
  radioGroupContainer: {
    borderWidth: 2, // Añade un borde al marco
    borderColor: COLORS.gray, // Color del borde
    borderRadius: 10, // Bordes redondeados
    padding: 10, // Espacio interior del marco
  ...
  radioButtonSelected: {
    backgroundColor: COLORS.primary, // Color del botón seleccionado
  },
  radioButtonText: {
    color: COLORS.black,
  },
...
export default InformeStandard;
