...
const Formulario = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchTitulosYPreguntas = async () => {
      try {
        setLoading(true);
        const responseTitulos = await fetch("https://isorga.com/api/titulosInforme.php?id=1");
        ...
        for (const titulo of titulosData) {
          const responsePreguntas = await fetch(`https://isorga.com/api/preguntasTitulo.php?id=${titulo.id}`);
  ...
  const handleAnswerChange = (preguntaId, answer, columna = null) => {
    ...
    setSelectedAnswers((prev) => ({
      ...prev,
      [preguntaId]: { ...prev[preguntaId], [columna || "respuesta"]: valorRespuesta },
    }));
  };

  // Función para seleccionar una imagen para cada respuesta
  const seleccionarImagen = async (preguntaId) => {
  ...
  // Función para validar que todas las preguntas de tipo "Sí/No" hayan sido respondidas
  const validarRespuestas = () => {
    const respuestasIncompletas = Object.keys(preguntas).some((tituloId) =>
      preguntas[tituloId].some(
        (pregunta) =>
          !selectedAnswers[pregunta.id] || selectedAnswers[pregunta.id].respuesta === undefined
      )
    );

    if (respuestasIncompletas) {
      Alert.alert('Error', 'Debe responder todas las preguntas de tipo "Sí" o "No" antes de continuar.');
      return false;
    }
  ...
  const enviarDatosAGuardarInforme = async () => {
    ...
    Object.keys(selectedAnswers).forEach((preguntaId) => {
      ...
      formData.append(`respuestas[${preguntaId}][respuesta]`, respuesta.respuesta);
      formData.append(`respuestas[${preguntaId}][observaciones]`, respuesta.observaciones || '');
      formData.append(`respuestas[${preguntaId}][responsable]`, respuesta.responsable || '');
    ...
    try {
      ...
      const response = await fetch('https://isorga.com/api/guardarInforme.php', {
        method: 'POST',
        body: formData,
        headers: {
  ...
  return (
    <SafeAreaView style={styles.area}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
              ...
              preguntas[titulo.id].map((pregunta) => (
                <View key={pregunta.id} style={styles.preguntaContainer}>
                  ...
                  <View style={styles.radioGroup}>
                    <TouchableOpacity
                      onPress={() => handleAnswerChange(pregunta.id, "SI")}
                      style={[
                        styles.radioButton,
                        selectedAnswers[pregunta.id]?.respuesta === 1 && styles.radioButtonSelected,
                      ]}
                    >
                      <Text style={styles.radioButtonText}>SI</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      onPress={() => handleAnswerChange(pregunta.id, "NO")}
                      style={[
                        styles.radioButton,
                        selectedAnswers[pregunta.id]?.respuesta === 0 && styles.radioButtonSelected,
                      ]}
                    >
                  ...
                  <TouchableOpacity onPress={() => seleccionarImagen(pregunta.id)} style={styles.button}>
                    <Text style={styles.buttonText}>Seleccionar Imagen</Text>
                  </TouchableOpacity>

                  {selectedAnswers[pregunta.id]?.image && (
                    <Image source={{ uri: selectedAnswers[pregunta.id]?.image }} style={styles.imagePreview} />
                  )}
                  ...
                  <TextInput
                    ...
                    placeholderTextColor={COLORS.gray}
                    onChangeText={(text) => handleAnswerChange(pregunta.id, text, "observaciones")}
                  />
                  <TextInput
                    style={styles.input}
                    placeholder="Responsable"
                    placeholderTextColor={COLORS.gray}
                    onChangeText={(text) => handleAnswerChange(pregunta.id, text, "responsable")}
                  />
                </View>
              ))}
          </View>
        ...
        <TouchableOpacity style={styles.submitButton} onPress={enviarDatosAGuardarInforme}>
