...
const FichaRequisito = ({ route, navigation }) => {
  ...
  const fetchMisGestiones = async () => {
    try {
      ...
      const response = await fetch(
        "https://isorga.com/api/fichaRequisito.php",
        {
          method: "POST",
          headers: {
  ...
  const renderHeader = () => {
    return (
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>FICHA REQUISITO</Text>
        <View style={{ flex: 1 }} />
    ...
    return (
        <View style={styles.card}>
          <Text style={styles.cardTitle}>
            {item.reqInterno} {item.condTexto}
          </Text>
