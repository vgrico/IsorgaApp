...
const PendientesLeer = ({ navigation }) => {
  ...
  const fetchMisGestiones = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://isorga.com/api/pendientesLeer.php", {
        method: "POST",
        headers: {
  ...
  const handleMarcarLeido = async (reqInterno) => {
    try {
      const response = await fetch("https://isorga.com/api/marcarLeido.php", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.ok) {
        // Recargar los datos después de marcar como leído
        fetchMisGestiones();
      } else {
        console.error("Error en la API marcarLeido");
      }
    } catch (error) {
  ...
  const renderHeader = () => {
    return (
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}> PENDIENTES DE LEER</Text>
        <View style={{ flex: 1 }} />
...
const truncateText = (text, wordLimit) => {
  ...
  const renderModulo = ({ item }) => {
    ...
    const isSelected = selectedCards.includes(item.reqInterno);
    ...
    return (
      <View style={styles.card}>
        ...
        <TouchableOpacity
            ...
            onPress={() => handleMarcarLeido(item.reqInterno)}
          >
