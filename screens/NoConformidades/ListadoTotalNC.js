...
const ListadoTotalNC = ({ navigation }) => {
  ...
  const fetchListadoNC = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://isorga.com/api/listadoNC.php", {
        method: "POST",
        headers: {
  ...
  const renderHeader = () => {
    return (
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>NC TOTALES</Text>
        <View style={{ flex: 1 }} />
