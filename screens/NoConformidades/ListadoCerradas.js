...
const ListadoCerradas = ({ navigation }) => {
  ...
  const fetchListadoNC = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://isorga.com/api/listadoCerradas.php", {
        method: "POST",
        headers: {
  ...
  const renderHeader = () => {
    return (
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>NC CERRADAS</Text>
        <View style={{ flex: 1 }} />
...
export default ListadoCerradas;
