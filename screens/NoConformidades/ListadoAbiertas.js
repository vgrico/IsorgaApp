...
const ListadoAbiertas = ({ navigation }) => {
  ...
  const fetchListadoNC = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://isorga.com/api/listadoAbiertas.php", {
        method: "POST",
        headers: {
  ...
  const renderHeader = () => {
    return (
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>NC ABIERTAS</Text>
        <View style={{ flex: 1 }} />
...
export default ListadoAbiertas;
