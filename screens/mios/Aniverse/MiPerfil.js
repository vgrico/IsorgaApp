...
import EstadisticasUsuario from '../../tabs/mios/EstadisticasUsuario';

const renderScene = (route, userId) => {
    switch (route.key) {
        case 'first':
            return <EstadisticasUsuario userId={userId} />;
        case 'second':
            return <CartasUsuarios userId={userId} />;
        case 'third':
            return <SeriesUsuarios userId={userId} />;
        default:
            return null;
    }
};
...
const MiPerfil = ({ navigation }) => {
    ...
    const fetchDatosUsuarios = async () => {
        try {
            const response = await fetch(`https://momdel.es/animeWorld/api/perfilUsuario.php?id=${userId}`);
    ...
    const fetchUsuariosSeguidores = async () => {
        try {
            const response = await fetch(
                `https://momdel.es/animeWorld/api/mirarSeguidores.php?id=${userId}`
            );
            const data = await response.json();
    ...
    const fetchSiSeguido = async () => {
        try {
            const response = await fetch('https://momdel.es/animeWorld/api/usuarioSiSeguido.php', {
                method: 'POST',
                headers: {
    ...
    const fetchSeguir = async () => {
        try {
            const response = await fetch('https://momdel.es/animeWorld/api/seguirUsuarios.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const progressBarWidth =
        datosUsuarios && datosUsuarios.diferencia !== null ? `${datosUsuarios.diferencia}%` : '0%';
            ...
            <View>
                <View style={styles.profileImageContainer}>
                    ...
                    <View style={styles.progressBarContainer}>
                        <Text style={styles.progressText}>⭐️ {datosUsuarios.nivel}</Text>
...
const styles = StyleSheet.create({
    ...
    modalTitle: {
        ...
        color: COLORS.primary,
        marginBottom: 16,
        textAlign: 'center', // Centrar el texto en el modal
    },
    modalImage: {
        width: '80%',
        height: '80%',
