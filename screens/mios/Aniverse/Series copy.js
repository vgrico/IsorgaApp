...
const Series = ({ navigation }) => {
    ...
    const [selectedTab, setSelectedTab] = useState('row') // Moved selectedTab state here
    const [searchQuery, setSearchQuery] = useState('')
    const [typingTimeout, setTypingTimeout] = useState(0)

    useEffect(() => {
        const fetchData = async () => {
            const characters = await listarSeries()
            setFilteredEvents(characters)
            setResultsCount(characters.length)
        }
        fetchData()
    }, [])
    ...
    const handleSearchChange = (text) => {
        ...
        setTypingTimeout(setTimeout(() => handleSearch(text), 500))
    }
    ...
    const renderFacilitiesItem = ({ item }) => (
        <TouchableOpacity
            style={{
                backgroundColor: selectedFacilities.includes(item.id)
                    ? COLORS.primary
                    : 'transparent',
                padding: 10,
                marginVertical: 5,
        ...
        >
            <Text
                style={{
                    color: selectedFacilities.includes(item.id)
                        ? COLORS.white
                        : COLORS.primary,
                }}
            >
        ...
        <SafeAreaView style={[styles.area, { backgroundColor: COLORS.white }]}>
            <View style={[styles.container, { backgroundColor: COLORS.white }]}>
                ...
                <ScrollView
                    ...
                    contentContainerStyle={{ paddingBottom: 50 }} // Ajusta este valor según sea necesario
                >
                    {renderContent()}
                </ScrollView>
                <RBSheet
