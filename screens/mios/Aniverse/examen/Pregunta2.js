...
const ExPregunta2 = ({ route, navigation }) => {
    ...
    const fetchPreguntas = async () => {
        try {
            const response = await fetch(
                `https://momdel.es/animeWorld/api/ExPregunta2.php?id=${id}`
            );
    ...
    const handleContinue = async () => {
        ...
        if (selectedOption.esCorrecta) {
            navigation.navigate('ExPregunta3', { id: id });
        } else {
