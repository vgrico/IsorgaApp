...
const CartasOrdenadas = ({ navigation }) => {
    ...
    const fetchCartas = async (userId) => {
        try {
            const response = await fetch(
                `https://momdel.es/animeWorld/api/cartasOrdenadasUsuario.php?id=${userId}`
            );
    ...
    const renderHeader = () => {
        return (
            <View style={styles.headerContainer}>
                ...
                <Text style={styles.headerTitle}>Por Rareza</Text>
            </View>
        );
    };
...
export default CartasOrdenadas;
