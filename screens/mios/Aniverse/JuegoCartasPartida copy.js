...
const JuegoCartasPartida = ({ route, navigation }) => {
    ...
    const [forceMachineAbilityInRound2, setForceMachineAbilityInRound2] = useState(false); // Nuevo estado
    ...
    const startRound = () => {
        if (rounds <= 6) {
            ...
            if (!machineAbilityUsed) {
                if (rounds === 1 && Math.random() < 1) {
                    utilizarHabilidad(machineAbility.id, false);
                    setMachineAbilityUsed(true);
                    return;
                } else if (rounds === 1) {
                    setForceMachineAbilityInRound2(true);
                }
            }
        ...
        if (updatedDefender.defensa <= 0) {
            if (isPlayer) {
                if (machineIndex + 1 < machineCards.length) {
                    ...
                    if (rounds === 2 && !machineAbilityUsed) {
                        utilizarHabilidad(machineAbility.id, false);
                        setMachineAbilityUsed(true);
                        setForceMachineAbilityInRound2(false);
                        return;
                    }
                    eleccion();
                } else {
