...
const DetallesPersonaje = ({ navigation, route }) => {
    ...
    const fetchMediaNota = async () => {
        try {
            const response = await fetch(
                `https://momdel.es/animeWorld/api/mediaSerie.php?id=${id}`
            )
            const data = await response.json()
            setMediaNotas(data)
        } catch (error) {
    ...
    const sliderImages = [
        ...
        serieData && serieData.length > 0 && serieData[0].imagen2,
        serieData && serieData.length > 0 && serieData[0].imagen3,
        serieData && serieData.length > 0 && serieData[0].imagen4,
    ]
            ...
            <View style={styles.contentContainer}>
                ...
                >
                    ...
                    <View style={styles.eventFeatureTextContainer}>
                        ...
                        <TouchableOpacity
                            onPress={() =>
                                navigation.navigate('EventDetailsLocation')
                            }
                ...
                <View style={styles.reviewContainer}>
                    <View style={styles.reviewLeft}>
                        ...
                        >
                            {mediaEstrellas &&
                                mediaEstrellas.length > 0 &&
                                mediaEstrellas[0].media}{' '}
                            ({resultsCount} reviews)
                        </Text>
                    </View>
                    <TouchableOpacity
                        onPress={() =>
                            navigation.navigate('ComentariosSerie', { id: id })
                        }
                    >
                        <Text style={styles.seeAll}>Ver Todos</Text>
                    </TouchableOpacity>
                ...
                <ComentariosCard
                    avatar={
                        comentarioData &&
                        comentarioData.length > 0 &&
                        comentarioData[0].imagen
                    }
                    name={
                        comentarioData &&
                        comentarioData.length > 0 &&
                        comentarioData[0].nombre
                    }
                    description={
                        comentarioData &&
                        comentarioData.length > 0 &&
                        comentarioData[0].texto
                    }
                    avgRating={
                        comentarioData &&
                        comentarioData.length > 0 &&
                        comentarioData[0].estrellas
                    }
                    date={
                        comentarioData &&
                        comentarioData.length > 0 &&
                        comentarioData[0].dia
                    }
                    numLikes={
                        comentarioData &&
                        comentarioData.length > 0 &&
                        comentarioData[0].likes
                    }
                    usuario={
                        comentarioData &&
                        comentarioData.length > 0 &&
                        comentarioData[0].usuarioId
                    }
                />
            </View>
        )
