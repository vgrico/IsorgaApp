...
const FormacionesPQ = ({ route, navigation }) => {
  ...
  const fetchDocumentos = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://isorga.com/api/formacionesPQ.php", {
        method: "POST",
        headers: {
  ...
  const handleSearch = (text) => {
    setSearch(text);
    const filtered = documentos.filter((item) =>
  ...
  const renderHeader = () => {
    return (
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>FORMACIONES P.Q.</Text>
        <View style={{ flex: 1 }} />
  ...
  const renderModulo = ({ item }) => {
    return (
      <TouchableOpacity
        onPress={() => navigation.navigate("FormacionPQ", { id: item.id })}
      >
