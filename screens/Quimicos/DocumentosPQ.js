...
const DocumentosPQ = ({ navigation }) => {
  ...
  const fetchDocumentos = async () => {
    try {
      setLoading(true);
      const response = await fetch("https://isorga.com/api/documentosPQ.php", {
        method: "POST",
        headers: {
  ...
  const renderHeader = () => {
    return (
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerTitle}>Documentos P.Q.</Text>
        <View style={{ flex: 1 }} />
  ...
  const renderModulo = ({ item }) => {
    return (
      <TouchableOpacity
        onPress={() => navigation.navigate("DocumentoPQ", { id: item.id })}
      >
